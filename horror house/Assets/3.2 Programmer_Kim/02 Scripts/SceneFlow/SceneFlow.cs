...
/// <summary>
/// 씬 전환 진입점. 모든 전환은 로딩 씬을 거친다.
/// 흐름: GoTo(목적지) → 로딩 씬 로드 → LoadingController가 목적지를 꺼내 비동기 로드.
/// </summary>
public static class SceneFlow
{
    ...
    /// <summary>로딩 씬을 거쳐 target 씬으로 이동한다.</summary>
    public static void GoTo(GameScene target)
    {
        // 버튼 연타로 전환이 중복 요청되는 것 방지
        if (isTransitioning) return;
        if (Config == null) return;

        string targetPath = Config.GetPath(target);
        string loadingPath = Config.GetPath(GameScene.Loading);
        if (!IsInBuild(targetPath, target) || !IsInBuild(loadingPath, GameScene.Loading)) return;
        ...
        SceneManager.LoadScene(loadingPath);
    }
