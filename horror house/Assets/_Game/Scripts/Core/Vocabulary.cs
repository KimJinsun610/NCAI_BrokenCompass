namespace NightDuty
{
    ...
    /// 확정 기획서의 4개 공간 / 5개 점검 ID(복도 · 1-1 · 1-3 · 과학실 · 화장실)와 정확히 일치한다.
    ...
    public enum SpaceId
    {
        ...
        /// <summary>
        /// 과학실. 등 4개. 조도 Band4(90~99)에서는 등 0개에 붉은 잔광만 남는다(공통 색온도표).
        /// </summary>
        ScienceRoom = 5
    }
    ...
    public enum FearAxis
    {
        ...
        /// <summary>
        /// 신뢰(信賴). 수칙을 <b>준수</b>할 때 오른다(+2~+4). 다른 세 축과 달리 월드에 절대 그려지지 않고 수치만 누적한다.
        /// 신뢰도 100에 도달하면 종료 잠금(게임오버)이다.
    ...
    /// Band0 = 0~24, Band1 = 25~49, Band2 = 50~74, Band3 = 75~89, Band4 = 90~99, 100 = 종료 잠금
    /// (Band3은 15칸, Band4는 10칸으로 뒤로 갈수록 좁아진다 — 고조 구간을 잘게 쪼개기 위한 의도적 설계다.)
    ...
    public enum Band
    {
        ...
        /// <summary>90~99. 엔딩 직전 경고 표현. 축이 100에 도달하면 구간이 아니라 종료 잠금이다(<c>EventBus.AxisCritical</c>).</summary>
        Band4 = 4
    }
}
